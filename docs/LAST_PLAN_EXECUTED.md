...
immediately, in parallel with the `webtyp/tui` and `webtyp/keyring` plans.

## Why

`webtyp/app` needs to scan every key in a project's `.env`-backed store
looking for the ones whose value marks them as living in `webtyp/keyring`
...
`grep -rn "kvdb\.KVStore" ~/Dev/Project/webtyp/*/*.go` (run before writing
this plan) shows exactly one embedder outside this repo:
`webtyp/app/interface.go` (`Handler.DB kvdb.KVStore`), which only ever
...
Nothing about `webtyp/keyring`, `webtyp/env`, or `webtyp/app` — those are
